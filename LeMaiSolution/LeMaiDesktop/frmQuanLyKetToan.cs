...
{
    public partial class frmQuanLyKetToan : frmBase
    {
        ...
        public frmQuanLyKetToan() : base(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType)
        {
            InitializeComponent();
        ...
        private async void LoadData(bool reload = false)
        {
            ...
            ExpPost post = _logicbill.GetPostDetail(PBean.USER.CardId);
        ...
        private void btnNapTien_Click(object sender, EventArgs e)
        {
            ...
            if (post.ParrentPost == "0000")
            {
                // Kiểm tra nếu là tài khoản cấp 1 nạp tiền là nạp tiền liên hệ thống
                frmQuanLyCongNoCap1 frmCongNo = new frmQuanLyCongNoCap1();
                frmCongNo.ShowDialog();
            }
            else
            {
                // Nếu ngược lại là cấp 2 nạp tiền cho cấp 1
                frmNewRequestMoney frm = new frmNewRequestMoney();
                frm.FK_PostTo = post.ParrentPost;
                frm.ShowDialog();
            }
        ...
        {
            // Nếu tài khoản là tài khoản cấp 1, thì không có chức năng rút tiền ẩn lúc load form rồi
            // Nếu tài khoản là tài khoản cấp 2, thì hiển thị form rút tiền.
            ExpPost post = _logicbill.GetPostDetail(PBean.USER.CardId);
            if (post.ParrentPost != "0000")
            {
                // Nếu ngược lại là cấp 2 nạp tiền cho cấp 1
                frmNewWithdrawMoney frm = new frmNewWithdrawMoney();
                frm.FK_PostTo = post.ParrentPost;
                frm.ShowDialog();
            }
        }
