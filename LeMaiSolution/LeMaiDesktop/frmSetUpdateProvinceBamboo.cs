...
namespace LeMaiDesktop
{
    ...
    public partial class frmSetUpdateProvinceBamboo : frmBase
    {
        int LoaiDuLieu = -1;
        ...
        public frmSetUpdateProvinceBamboo() : base(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType)
        {
            InitializeComponent();
        }
                ...
                if (LoaiDuLieu == 0)
                {
                    ...
                    for (int i = 0; i < gridata.Rows.Count; i++)
                    {
                        ...
                        if (gridata.Rows[i].Cells["col_Select"].Value != null)
                        {
                            ...
                            GExpProvinceBamboo provincevnp = dc.GExpprovincebamboo.GetObject(PBean.SCHEMA, provinceVNPTId);
                            if (provincevnp != null)
                            {
                                provincevnp.ProvinceID = Int32.Parse(provinceId);
                                dc.GExpprovincebamboo.Update(PBean.SCHEMA, provincevnp);
                            }
                        }
                ...
                else if (LoaiDuLieu == 1)
                {
                    ...
                    for (int i = 0; i < gridata.Rows.Count; i++)
                    {
                        ...
                        if (gridata.Rows[i].Cells["col_Select"].Value != null)
                        {
                            ...
                            GExpDistrictBamboo district = dc.GExpdistrictbamboo.GetObject(PBean.SCHEMA, districtVNPTId);
                            if (district != null)
                            {
                                district.DistrictID = Int32.Parse(districtId);
                                dc.GExpdistrictbamboo.Update(PBean.SCHEMA, district);
                            }
                        }
                ...
                else if (LoaiDuLieu == 2)
                {
                    ...
                    for (int i = 0; i < gridata.Rows.Count; i++)
                    {
                        ...
                        if (gridata.Rows[i].Cells["col_Select"].Value != null)
                        {
                            ...
                            GExpWardBamboo ward = dc.GExpwardbamboo.GetObject(PBean.SCHEMA, wardVNPTId);
                            if (ward != null)
                            {
                                ward.WardId = wardId.Trim();
                                dc.GExpwardbamboo.Update(PBean.SCHEMA, ward);
                            }
                        }
        ...
        private void btnLoadTinh_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                List<GExpProvinceBamboo> lsProvince = dc.GExpprovincebamboo.GetListObjectCon(PBean.SCHEMA, "ORDER BY provinceName");
        ...
        private void btnLoadHuyen_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                List<GExpDistrictBamboo> lsData = dc.GExpdistrictbamboo.GetListObjectCon(PBean.SCHEMA, "WHERE provinceCode=@provinceCode ORDER BY districtName",
                    "@provinceCode", cmbTinh.SelectedValue.ToString());

                DataGridViewComboBoxColumn dgvCmb = new DataGridViewComboBoxColumn();
                dgvCmb.HeaderText = "Huyện tương ứng";
                dgvCmb.DataSource = dc.GExpdistrict.GetListObjectCon(PBean.SCHEMA, " INNER JOIN GExpProvince B ON A.ProvinceID = B.ProvinceID INNER JOIN GExpProvinceBamboo C ON C.ProvinceID = B.ProvinceID WHERE C.provinceCode=@provinceCode",
                    "@provinceCode", cmbTinh.SelectedValue.ToString());
                _lsDistrict = dc.GExpdistrict.GetListObjectCon(PBean.SCHEMA, " INNER JOIN GExpProvince B ON A.ProvinceID = B.ProvinceID INNER JOIN GExpProvinceBamboo C ON C.ProvinceID = B.ProvinceID WHERE C.provinceCode=@provinceCode",
                    "@provinceCode", cmbTinh.SelectedValue.ToString());
        ...
        private void btnLoadXa_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                List<GExpWardBamboo> lsData = dc.GExpwardbamboo.GetListObjectCon(PBean.SCHEMA, "WHERE districtCode=@districtCode ORDER BY communeName",
                    "@districtCode", cmbHuyen.SelectedValue.ToString());

                DataGridViewComboBoxColumn dgvCmb = new DataGridViewComboBoxColumn();
                dgvCmb.HeaderText = "Xã tương ứng";
                dgvCmb.DataSource = dc.GExpward.GetListObjectCon(PBean.SCHEMA, " INNER JOIN GExpDistrict B ON A.DistrictID = B.DistrictID INNER JOIN GExpDistrictBamboo C ON C.DistrictID = B.DistrictID WHERE C.districtCode=@districtCode",
                    "@districtCode", cmbHuyen.SelectedValue.ToString());
                _lsWard = dc.GExpward.GetListObjectCon(PBean.SCHEMA, " INNER JOIN GExpDistrict B ON A.DistrictID = B.DistrictID INNER JOIN GExpDistrictBamboo C ON C.DistrictID = B.DistrictID WHERE C.districtCode=@districtCode",
                    "@districtCode", cmbHuyen.SelectedValue.ToString());
        ...
        private void cmbTinh_SelectedValueChanged(object sender, EventArgs e)
        {
            if (ActiveControl == cmbTinh)
            {
                if (cmbTinh.SelectedValue != null)
                {
                    ...
                    {
                        dc.Open();
                        List<GExpDistrictBamboo> lsDistrict = dc.GExpdistrictbamboo.GetListObjectCon(PBean.SCHEMA, "WHERE provinceCode = @provinceCode ORDER BY districtName",
                            "@provinceCode", cmbTinh.SelectedValue.ToString());
        ...
        private void btnMapAll_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    List<GExpWardBamboo> lsWardVNP = dc.GExpwardbamboo.GetListObjectCon(PBean.SCHEMA, "WHERE districtCode=@districtCode", "@districtCode", item.districtCode);
                            ...
                            if (string.IsNullOrEmpty(wardVNPT.WardId))
                            {
                                ...
                                foreach (var ward in lsWard)
                                {
                                    ...
                                    if (wardVNPTName.Equals(wardName) == true)
                                    {
                                        ...
                                        dc.GExpwardbamboo.Update(PBean.SCHEMA, wardVNPT);
                                        found = true;
                                        break;
                                    }
                                }
        ...
        private void btnDuLieuHuyen_Click(object sender, EventArgs e)
        {
            ...
            if (open.ShowDialog() == DialogResult.OK)
            {
                ...
                try
                {
                    ...
                    {
                        GExpProvinceBamboo pro = dc.GExpprovincebamboo.GetObjectCon(PBean.SCHEMA, "WHERE provinceName=@provinceName", "@provinceName", row["Province"].ToString().Trim());
                        if (pro == null)
                        {
                            provId++;
                            pro = new GExpProvinceBamboo();
                            pro.provinceName = row["Province"].ToString().Trim();
                            pro.provinceCode = provId.ToString();
                            dc.GExpprovincebamboo.InsertOnSubmit(PBean.SCHEMA, pro);
                        }
                        GExpDistrictBamboo dis = dc.GExpdistrictbamboo.GetObjectCon(PBean.SCHEMA, "WHERE districtName=@districtName AND provinceCode=@provinceCode",
                            "@districtName", row["District"].ToString().Trim(),
                            "@provinceCode", pro.provinceCode);
                        if (dis == null)
                        {
                            distId++;
                            dis = new GExpDistrictBamboo();
                            dis.districtCode = distId.ToString();
                            dis.districtName = row["District"].ToString().Trim();
                            dis.provinceCode = pro.provinceCode;
                            dc.GExpdistrictbamboo.InsertOnSubmit(PBean.SCHEMA, dis);
                        }

                        GExpWardBamboo ward = dc.GExpwardbamboo.GetObjectCon(PBean.SCHEMA, "WHERE communeName=@communeName AND districtCode=@districtCode",
                            "@communeName", row["Area"].ToString().Trim(),
                            "@districtCode", dis.districtCode);
                        if (ward == null)
                        {
                            wardId++;
                            ward = new GExpWardBamboo();
                            ward.communeCode = wardId.ToString();
                            ward.communeName = row["Area"].ToString().Trim();
                            ward.districtCode = dis.districtCode;
                            ward.PortCode = row["Code"].ToString().Trim();
                            dc.GExpwardbamboo.InsertOnSubmit(PBean.SCHEMA, ward);
                        }
                    }
                    dc.SubmitChanges();
                    MessageBox.Show("Hoàn thành lấy dữ liệu từ Bamboo thành công", PBean.MESSAGE_TITLE);
                }
                catch (Exception ex)
                {
                    throw ex;
        ...
        private void btnXoaXa_Click(object sender, EventArgs e)
        {
            if (cmbHuyen.SelectedValue != null)
            {
                ...
                {
                    dc.Open();
                    dc.ExecuteNonQuery("UPDATE GExpWardBamboo SET WardId=NULL WHERE districtCode=@districtCode",
                        "@districtCode", cmbHuyen.SelectedValue.ToString());
                    dc.SubmitChanges();
                }
                catch (Exception ex)
