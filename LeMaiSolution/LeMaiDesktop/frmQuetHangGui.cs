...
{
    public partial class frmQuetHangGui : frmBase
    {
        ...
        public frmQuetHangGui() : base(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType)
        {
            InitializeComponent();
        ...
        {
            // Xóa kiện vấn đề
            if (MessageBox.Show("Bạn có muốn xóa gửi đơn hàng này không?", PBean.MESSAGE_TITLE, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    int index = gridBills.SelectedRows[0].Index;
                    string IdTrack = Convert.ToString(gridBills.Rows[index].Cells["col_Id"].Value);
                    _logicScan.DelSend(IdTrack);
                    LoadData();
                }
            ...
            if (e.KeyChar == (char)Keys.Enter)
            {
                if (!string.IsNullOrEmpty(txtMaDonHang.Text))
                {
                    ...
                    if (bill != null)
                    {
                        ...
                        string message = _logicScan.AddSend(bill.BillCode, PBean.USER.Id, PBean.USER.FullName, PBean.USER.Phone, cmbPost.SelectedValue.ToString(), PBean.USER.CardId);
        ...
        public async void LoadData(bool reload = false)
        {
            ...
            var list = _logicScan.GetScanSendListFilter(PBean.USER.CardId, cmbFilter.SelectedValue.ToString(), dateFrom.Value, dateTo.Value, txtBillCode.Text.Trim());
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            if (list.Count > 0)
            {
                ...
                save.FileName = "QUET_GUI_" + string.Format("{0:dd_MM_yyyy_(HH_mm)}", DateTime.Now);
                save.Filter = "Excel Files | *.xlsx";
                if (save.ShowDialog() == DialogResult.OK)
                {
                    string templateFileName = AppDomain.CurrentDomain.BaseDirectory + "Templates\\MAU_QUET_GUI.xlsx";
